...
const IPHONE_X_WEB_COORD_OFFSET = 25;
...
const getSafariIsIphoneX = _.memoize(async function getSafariIsIphone (sessionId, driver) {
  // sessionId parameter is for memoizing per session
  // because of the way lodash memoization works, this function cannot call
  // `getSafariIsIphone` and short-circuit when not on an iPhone
  let isIphoneX = false;
  try {
    const height = await driver.execute('return window.screen.availHeight');
    const width = await driver.execute('return window.screen.availWidth');
    // check for the correct height and width
    isIphoneX = (height === IPHONE_X_HEIGHT && width === IPHONE_X_WIDTH) ||
...
extensions.getExtraTranslateWebCoordsOffset = async function (coords, webviewRect) {
  ...
  try {
    ...
    if (await getSafariIsIphoneX(this.opts.sessionId, this)) {
      offset += IPHONE_X_EXTRA_WEB_COORD_NON_SCROLL_OFFSET;
    } else if (await getSafariIsIphone(this.opts.sessionId, this)) {
  ...
  offset += await getSafariIsIphoneX(this.opts.sessionId, this) ?
    IPHONE_X_WEB_COORD_OFFSET : 0;
...
async function tapWebElementNatively (driver, atomsElement) {
  ...
  try {
    ...
    if (text) {
      ...
      await driver.clickCoords({x: rect.x + rect.width / 2, y: rect.y + rect.height});
      return true;
    }
  } catch (err) {
